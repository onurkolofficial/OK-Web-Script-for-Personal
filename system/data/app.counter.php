...
$ApplicationsResult=$Query->Query($Server, "SELECT * FROM applications ORDER BY `applications`.`app_release_date` DESC");
// Set Variables
...
function getAppCountFromCategory($Server, $Query, $CategoryId){
    $caResult=$Query->Query($Server, "SELECT * FROM applications WHERE category_id='$CategoryId' ORDER BY `applications`.`app_release_date` DESC");
